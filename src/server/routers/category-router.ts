...
export const categoryRouter = router(
    {
        getEventCategories: privateProcedure.query(async ({ c, ctx }) => {
            ...
            const categoriesWithCounts = await Promise.all(
                categories.map(async (category) => {
                    ...
                    const [uniqueFieldsCount, eventCounts, lastPing] = await Promise.all([
                        db.event.findMany({
                            where: {
                    ...
                    return {
                        ...category,
                        uniqueFieldsCount, eventCounts, lastPing: lastPing?.createdAt || null
                    }
                })
            );
            return c.superjson({
        ...
        insertQuickStartCategory: privateProcedure.mutation(
            async ({ ctx, c }) => {
