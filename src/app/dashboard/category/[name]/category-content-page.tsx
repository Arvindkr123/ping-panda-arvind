...
export const CategoryPageContent = ({ hasEvents: initialHasEvents, category }: CategoryPageContentProps) => {
    ...
    const NumericeFieldsCards = () => {
        if (Object.keys(numericFieldsSum).length === 0) {
            return null;
        }
        return Object.entries(numericFieldsSum).map(([field, sums]) => {
            ...
            return <Card key={field}>
                ...
                <div className="">
                    ...
                    <p className="text-xs/5 text-muted-foreground">{activeTab === 'today' ? 'today' : activeTab === 'week' ? 'this week' : "this month"}</p>
                </div>
            </Card>
        })
    }
    ...
    const table = useReactTable(
        {
            ...
            pageCount: Math.ceil((data?.events.length || 0) / pagination.pageSize),
