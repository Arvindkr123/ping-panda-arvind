...
const REQUEST_VALIDATOR = z.object({
    ...
    fields: z.record(z.string().or(z.number()).or(z.boolean())),
...
export const POST = async (req: NextRequest) => {
    try {
        ...
        const dmChannel = await discordClient.createDM(user.discordId);

        // await discordClient.sendEmbed(dmChannel.id, {
        //     title:'Hello world !!!'
        // })
        ...
        if (!category) {
            return NextResponse.json({
                message: `You do not have category name "${validationResult.category}"`
            }, {
                status: 404
            })
        }
        ...
        try {
            await discordClient.sendEmbed(dmChannel.id, eventData);
