...
export const CreateEventCategoryModal = ({ children, contaierClassName }: CreateEventCategoryModal) => {
    ...
    const { mutate: createEventCategory, isPending: isEventCategoryCreated } = useMutation({
    ...
    return (
        <>
            <div className={contaierClassName} onClick={() => setIsOpen(true)}>
                {children}
            </div>
            <Modal className='max-w-xl p-8' showModal={isOpen} setShowModal={setIsOpen}>
                <form onSubmit={handleSubmit(onSubmit)} className='space-y-6'>
                            ...
                            <div className="flex flex-wrap gap-3">
                                {
                                    COLOR_OPTIONS.map((premadeColor) => (
                                        <button onClick={() => setValue('color', premadeColor)} key={premadeColor} type='button' className={cn(`
                                            bg-[${premadeColor}]`,
                            ...
                            <div className="flex flex-wrap gap-3">
                                {
                                    EMOJI_OPTIONS.map(({ emoji, label }) => (
                                        <button onClick={() => setValue('emoji', emoji)} key={emoji} type='button' className={cn(
                    ...
                    <div className="flex justify-end space-x-3 pt-4 border-t">
                        ...
                        <Button disabled={isEventCategoryCreated} type='submit'>{isEventCategoryCreated ? 'Creating...' : 'Submit'}</Button>
                    </div>
                </form>
            </Modal>
        </>
